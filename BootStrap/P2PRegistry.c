...
int main(int argc, char** argv) {
	...
	if (retval <= 0 || !(portno >= 1024 && portno <= 65535)) {
		printf(
				"Invalid port number. Port number should be between 1024-65535\n");
		exit(1); // exit with error
	}
...
int opensocket(int portno) {
	...
	//bind to port
	retval = bind(sockfd, (struct sockaddr *) &myaddress,
...
void receivedata(int sockfd) {
	...
	connectedsocket = accept(sockfd, (struct sockaddr *) &senderaddress,
			&addresslen);
	...
	if (strcmp("REG", string_token) == 0) //Check whether the command is valid.
		...
		for (i = 0; i < TABLE_SIZE; i++) {
			if ((strncmp(peer_table[i].key, string_token, 8) == 0)
					&& (peer_table[i].valid == 1)) {
...
int sendpeerlist(int sockfd, char key[8]) {
	char message[1000], tmpmsg[100], tmpmsglen[4], key2[8];
	int msglen, numbytessend, retvalue, i, j, count;
...
unsigned char checkpeertable(char key[8]) {
	int i;
	for (i = 0; i < TABLE_SIZE; i++)
		if ((strncmp(peer_table[i].key, key, 8) == 0)
				&& (peer_table[i].valid == 1))
			return 1;
	return 0;
}
